...
\node[fill=yellow!30] at (mon |- p1) {
	\textbf{Обычная пара}\\[1em](Что)\\Где\\[1em]Какая группа
};

\node[dist,fill=orange!20] at (mon |- p2) {
	\textbf{Дистанционная пара}
};

\node[fill=magenta!15] at (mon |- 0,{t2f(14, 0)}) {
	\textbf{Пара точно в 14:00}
};

\node[prac,fill=pink!40] at (tue |- p1) {
	\textbf{Практикум}\\[1em]Что\\Где/какие задачи\\[1em]Какая группа
};

\node[greyedout] at (tue |- p3) {
	\textbf{Нет пары}\\[1em]Почему?\\Загадка\dots
};

\node[fill=green!20] at (wed |- p1) {
	\textbf{Мигающая пара}\\[0.3em]Где\\[0.3em]Какая группа\\\hfill {\scriptsize (Не)чётные недели}\hspace*{0.3em}
};

\node[fill=blue!10] (blink) at (thu |- p1) {
	\textbf{Мигающая пара 1}\\Где\\Какая группа\\[0.2em]
	\textbf{Мигающая пара 2}\\Где\\Какая группа\\
};
\draw (blink.0) -- (blink.180);
